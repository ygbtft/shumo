...
数学定义见 [Q1/Q2 实现说明](../../../../docs/q1q2-design.md) 和 [模型正文](../../../../paper/modeling-q1q2.md)。本目录运行离线数学模型，记录参数、物理假设与搜索分辨率的影响。

- [T6 执行记录](T6.md)：S1—S9 完成范围、样本/网格/评分次数、公共补点、扫描比例、细化、排序、时间与停止原因。
- [论文表格与结论段落草稿](paper-draft.md)：供论文正文引用。
...
S1/S2/S3/S4/S7 和复用 F11/F9 的 S8/S9 执行；S5/S6 为选做未执行。1.01°压力和精确量化未执行。各场景的参照模型只计算一次供相关参数实验复用；F11/F9 保存为可绘制数值数据。

搜索仍调用原 `select_second_point`，站点50/25 m，局部最小步长1.5625 m。精简源三级为 `(5,13)/(9,25)/(17,49)`，保留2个分散起点并保留对称侧/边界起点；边界射线15°，点对4起点/4轮。每次搜索30 s。具体搜索配置以本段和 `results/manifest.json` 为准。每模型的旧点、搜索推荐点、原搜索最小评分点与镜像点，先判可行，再在同一第三级源池、错位源及各点接触/合法见证的公共并集上终评。统一tie阈值0.1 m。
...
```sh
.venv/bin/python -m algorithms.q1q2.sensitivity --output algorithms/q1q2/reviews/sensitivity/results-replay --search-seconds 30
.venv/bin/python -m algorithms.q1q2.reviews.sensitivity.supplement algorithms/q1q2/reviews/sensitivity/results-replay
.venv/bin/python -m algorithms.q1q2.reviews.sensitivity.budget_threshold algorithms/q1q2/reviews/sensitivity/results-replay
.venv/bin/python -m algorithms.q1q2.reviews.sensitivity.report algorithms/q1q2/reviews/sensitivity/results-replay
.venv/bin/python -m pytest algorithms/q1q2/tests -q --import-mode=importlib
.venv/bin/python algorithms/q1q2/reviews/sensitivity/verify.py
```
...
```sh
.venv/bin/python -m algorithms.q1q2.reviews.sensitivity.pack algorithms/q1q2/reviews/sensitivity/results-replay
.venv/bin/python -m algorithms.q1q2.reviews.sensitivity.check_artifacts algorithms/q1q2/reviews/sensitivity/results-replay
```
