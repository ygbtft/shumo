...
def prepare(out,scope,repeats):
    ...
    # A new batch must use the current production registry and matching adapters.
    # Existing batches retain their own immutable package and configuration.
    for name in ('bounded_candidates.py', 'param_sensitivity.py', 'component_ablation.py',
                 'ablation_data_suite.py', 'official_ablation_robot.py'):
        shutil.copy2(ROOT/name, package/'B'/name)
