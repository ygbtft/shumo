...
def load_cases():
    ...
    for p, folder in [(3, 'q3-nearest-20260913'), (4, 'q4-ablation-metrics-20260913')]:
        path = ROOT / 'guest_deploy_evidence' / folder / f'q{p}_per_case_metric_audit.json'
        old.update({c['case_code']: c for c in json.loads(path.read_text())['cases']})
    for batch, name in SOURCES.items():
        ...
        for run in raw:
            ...
            if batch == 'ablation':
                trace = Path(old[a['case_code']]['trace'])
            else:
                paths = list(Path(run['evidence']).rglob('requests.jsonl'))
                assert len(paths) == 1, (batch, a['case_code'], paths)
                trace = paths[0]
...
def make_tables(cases):
    ...
    baselines = {
        3: summarize('q3_baseline', 'baseline：最近邻＋65米＋2轮', q3, 'baseline', None),
        4: summarize('q4_baseline', 'baseline：最近邻＋35米＋默认参数', q4, 'baseline', None),
    }
    labels = [('full', '原完整策略'), ('online_nearest', '最近邻任务排序'),
    ...
    for p in (3, 4):
        ...
        for gate in ([20, 80] if p == 3 else [20, 40, 80]):
            ...
            cs = pick(batch='ablation', problem=p, setting=variant)
            assert len(cs) == 10
            rows.append(summarize(variant, f'原完整策略，{gate}米', cs, '原消融', baselines[p]))
        tables.append(dict(number=p-2, problem=p, title=f'Q{p} 消融实验及正式采用方案对照', rows=rows))
    config = json.loads((RUNS / SOURCES['sensitivity'] / 'config.json').read_text())
    for p in (3, 4):
        rows = [baselines[p]]
        ...
        for setting, label, n in [('baseline', '原排序默认参数（联合扰动同期对照）', 40),
                                  ('perturbed', '原排序联合随机扰动（每局参数不同）', 80)]:
...
def markdown_table(t):
    ...
    if t['number'] == 1:
        lines += ['原消融各行以原完整策略为实验起点；除门限行外均为50米、主定位预算3轮。新增baseline来自两轮各5局融合方案演练，改变了排序、门限和预算。历史行不能重新解释为围绕融合baseline的单机制消融。']
    elif t['number'] == 2:
        lines += ['原消融基于原完整策略、35米及默认参数；最近邻行的10局作为本次统一baseline。其他消融行没有全部改成最近邻排序，20/40/80米行亦采用原排序。']
    else:
        lines += ['“原排序”指历史完整策略的任务排序（two_opt）。单参数宽扫仅改变行中所列参数，其余保持历史默认；本次将正式采用方案作为统一比较基准，不改变历史实验配置。']
        if t['number'] == 3:
            lines += ['历史默认：门限50米、share_limit=6、max_active=3、localization_weight=0.08、remainder_weight=1.5。中止批次仅完成对照3局、候选4局，全部保留，不能当作完整复验。baseline两轮5局已合并，未重复添加这10局。']
        else:
            lines += ['历史默认：门限35米、share_limit=6、share_cooldown=150米、transverse_m=40米、fraction=0.15、steps=10、pause_limit=16。后续同配置5局独立列示，不与原10局混并；联合扰动80局是多种配置的汇总，不是某一个参数方案。']
    return '\n'.join(lines) + '\n'
...
def formal_provenance():
    ...
    for p, folder in [(3, 'q3-fused-formal-20260913'), (4, 'q4-nearest-formal-20260913')]:
        base = ROOT / 'guest_deploy_evidence' / folder
        ...
        assert ('第 2 次' if p == 3 else 'formal_index=2') in report.read_text()
...
问题三的baseline为最近邻任务排序、试清门限65米、主定位预算2轮，其他参数为顺路补测上限6、定位代价权重0.08、余程权重1.5。该配置对应第二次正式案例`3VUE-GVZH-MPKV-GBWE`。统计样本取正式测试前两轮各5局演练，合计10局；两轮全部保留，不只选取成绩较好的第二轮。原“最近邻＋50米＋3轮”保留为历史对照。
...
本次共纳入927个不同的官方演练案例：消融160局、参数宽扫500局、联合扰动240局、Q3融合方案10局、Q3中止复验7局、Q4融合参数候选5局、Q4默认参数最近邻复测5局。另有4局接入校验未计入结果表。表1与表3重复展示同一组Q3基准，表2与表4重复展示同一组Q4基准，不能把四表行数乘以局数当成独立总样本量。本次未重新调用模拟器，未混入本地mock结果。
...
原消融与参数宽扫大多围绕历史原排序配置展开。下表更换的是描述性比较基准，没有改变历史实验的其余参数；因此，表格名称采用“消融实验及正式采用方案对照”和“参数实验及追加复测结果”，避免将历史试验误写为围绕最终方案重新实施的单因素实验。
...
'''
    ...
    text += (
        ...
        f'历史最近邻50米方案的平均定位清除时间为{val(1, "online_nearest__g50")}秒，'
        ...
        '顺路补测、门限和定位预算的单参数结果均来自原排序，不能将各自观测收益直接相加来预测融合方案。\n\n'
        f'问题四的参数结果见表4。在原排序宽扫中，纵向探测比例0.30、顺路补测上限2分别得到'
        ...
        '该追加比较与保留默认参数的选择相符，但每组仅5个不同案例，只能作为描述性证据。\n\n'
        '联合扰动数据在表3、表4末尾保留。按本材料的逐局等权口径，Q3扰动组和同期原默认对照的'
...
- 原排序同默认配置在不同批次的平均定位清除时间已有变化：Q3消融为247.50秒、参数宽扫为263.93秒；Q4分别为509.25秒和451.31秒。不能把不同批次之间的全部差异解释为参数或排序效果。
...
- `experiments/runs/2026-09-13_official-ablation`：160局正式纳入的消融演练及2局接入。
- `experiments/runs/2026-09-13_official-sensitivity`：500局参数宽扫、240局联合扰动及2局接入；原冻结分析见`PLAN.md`、`analysis_plan.json`、`REPORT.md`、`robustness.json`。
- `experiments/runs/2026-09-13_official-q3-fused-five`与`...-repeat`：Q3融合方案两轮各5局。
- `experiments/runs/2026-09-13_official-q3-refinement`：Q3实际完成的7局参数复验。
- `experiments/runs/2026-09-13_official-q4-fused-five`与`...-q4-nearest-five`：Q4两个后续候选各5局。
- `guest_deploy_evidence/q3-fused-formal-20260913`、`guest_deploy_evidence/q4-nearest-formal-20260913`：第二次正式测试方案身份和冻结参数，不作为表中均值样本。
...
`.tex`文件为四表的LaTeX排版输入，可在加载`ctex`、`longtable`、`booktabs`、`array`的文档中使用；表号由论文实际位置自动编排。使用时须同时保留对应Markdown中的表注，尤其是历史默认配置、实际局数与比较基准说明。
'''
    (OUT / '论文素材.md').write_text(text)
    kit = ROOT.parent / 'writing-kit'
    kit.mkdir(exist_ok=True)
    (kit / 'kit-q34.md').write_text(
        '# 问题三、四论文素材：第二次正式测试方案\n\n'
        '当前模型推导见[建模说明](../models/q34/modeling-q34.md)，'
        '运行入口见[当前代码说明](../B/Q34_CURRENT.md)。'
        '以下四表与正文由同一复算脚本生成；可下载表格及审计文件位于'
        '[四表目录](../B/experiments/paper_materials/2026-09-13_formal2_baselines/四表汇总.md)。\n\n' + text)
    ...
    (kit / 'kit-optimization.md').write_text(
        ...
        '[Q3/Q4论文素材](kit-q34.md)。本文件替换此前基于旧版本与mock数据的优化素材。\n\n' + analysis)
...
def main():
    with localcontext() as ctx:
        ...
        for t in tables:
            ...
            (OUT / (stem + '.md')).write_text(markdown_table(t))
            (OUT / (stem + '.tex')).write_text(latex_table(t))
