"""Fresh confirmation of wide-pair, bounded-width and completion-sensing policies."""
import argparse
import json
import math
import os
...
OUT = ROOT / "experiments/runs/2026-09-11_wide-confirmation"
SPECS = {3: {
    ...
    "expanded_area_return1_7": dict(kind="completion", layout="ring7", area_prior=True, remainder_weight=1., expanded=True),
}, 4: {
...
def cases(problem):
    ...
    for li, layout in enumerate(("boundary_outward", "boundary_tangent", "near_collinear", "cluster_far20", "range_transition")):
        for count in (10, 13, 16):
            rng = np.random.default_rng(np.random.SeedSequence([42, 116, problem, li, count]))
            ...
            for sign in ("positive", "negative", "spatial"):
                seed = 42 + 2000 + 100 * problem + index
                index += 1
                scenario = Scenario(seed, tuple(sources), dict(stress_layout=layout, source_rng=[42, 116, problem, li, count]))
                yield f"{layout}__n{count}__{sign}", layout, "stress", scenario, ErrorConfig(model="adversarial", adversarial_sign=sign)


def freeze(paths):
    import hashlib
    checked_runs = (("wide-probes", 1674, 285), ("completion-width", 1581, 267))
    for name, count, checks in checked_runs:
        ...
        assert tests["passed"] == checks and tests["failed"] == 0
        for filename in ("wide_probe_policy.py", "interleaved_policy.py", "policies.py", "geometry.py", "client.py"):
            assert hashlib.sha256((ROOT / filename).read_bytes()).hexdigest() == config["code_sha256"][filename]
        if name == "completion-width":
            for filename in ("completion_sensing_policy.py", "bounded_width_policy.py"):
                assert hashlib.sha256((ROOT / filename).read_bytes()).hexdigest() == config["code_sha256"][filename]
    _prior_freeze(paths)
    config = json.loads((OUT / "run_config.json").read_text())
    config.update(confirmation_seeds=list(range(107, 117)), expected_executions=2145,
        derivation="42+65+repeat,0..9; not generated in earlier runs",
        stress_derivation="SeedSequence([42,116,problem,layout_index,count]); errors42+2000+100*problem+index",
        untouched_future_seeds=list(range(117, 127)), freeze_before_truth_generation=True,
        engine_reuse="icra_confirmation.run with explicit OUT/SPECS/builders/all_paths/cases/freeze/summarize replacements",
        selection="3255 completed training executions; Q3 two prior/completion variants for mean versus tail; Q4 three existing baselines and four wider/width candidates; no confirmation-driven parameter changes",
        virtual_upper_bound_s=335136, instruction_upper_bound=9766,
        wide_proof="WIDE_PROBE_GUARANTEE.md", expanded_q3_action_rectangle=[-150.,1652.,-277.,277.])
    write_json(OUT / "run_config.json", config)
    (OUT / "precheck.md").write_text("# 新场景前检查与冻结\n\n宽角度285项、面积矩/横移267项检查通过；两轮3255次计分全清。训练决策文件哈希未变。本轮11候选先冻结，再生成107—116及新压力流；2145次、正式请求0。面积先验只影响排名，宽角度仍满足成对负反馈充分条件；预算335136s/9766。旧采集字段冲突已检查：当前构造器没有initialization_s统计字段，初始化由外层独立测量。\n")


def summarize(rows):
    _prior_summary(rows)
    path = OUT / "summary.md"
    text = path.read_text().replace("seed42派生77—86", "seed42派生107—116").replace("Q4旧方格保守", "Q4已有四分之一探测")
    text = text.replace("# 冻结后确认：未用于调参的场景", "# 宽探测与完工评分：冻结后确认")
