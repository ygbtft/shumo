"""Compute paired coupled-policy effects without changing frozen candidates."""
import csv
import json
from pathlib import Path
import numpy as np
from coupled_confirmation import OUT, SPECS
...
def main():
    ...
    assert completion["executions"] == completion["all_cleared"] == 1950
    rows = [json.loads(line) for line in (OUT / "trials.jsonl").read_text().splitlines()]
    summary = list(csv.DictReader((OUT / "summary.csv").open()))
    comparisons={3:tuple((m,b) for m in ("range_area7","fast_locked_area7") for b in ("area_return1_7","peer_layout9_proxy")),
                 4:tuple((m,b) for m in ("range_width015","fast_arc05_width015","fast_locked_width015","fast_negative_arc05_width015")
    ...
    for folder_name, title in (("coupled-dispatch","扫描顺序与服务入口/出口：已见训练比较"),("negative-hull","负反馈排除锥：已见训练比较"),("fast-dispatch","等价路线加速：已见训练比较")):
