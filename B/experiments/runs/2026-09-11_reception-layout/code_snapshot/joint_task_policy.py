...
class JointTaskPolicy(BracketMixin,JointPolicy):
    def __init__(self,client,stations,mixed=False,task_order="two_opt",share=True,
        ...
        self._sharing=False;self._surveying=False
    ...
    def complete_source(self,ch):
        if self.mixed:BracketMixin.complete_source(self,ch)
        else:JointPolicy.complete_source(self,ch)
    ...
    def share_at(self,p,exclude):
        ...
        for ch in self.regions:
            ...
            if distance>1000.+radius:continue  # optional usefulness filter only
            angle=math.radians(self.observations[ch][0][1]);u=np.array([math.cos(angle),math.sin(angle)])
    ...
    def run(self):
        self.client.enter();unused=list(range(len(self.stations)))
        while len(self.cleared)<16:
            ...
            if kind=="source":
                ...
                if key not in self.cleared:raise RuntimeError("Source completion did not terminate")
                continue
            unused.remove(key);self.stats["stations_visited"]+=1;self._surveying=True
            try:
