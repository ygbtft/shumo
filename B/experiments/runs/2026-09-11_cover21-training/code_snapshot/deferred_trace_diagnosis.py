...
from deferred_confirmation import OUT, SPECS, all_paths, build
...
def main():
    ...
    selected={}
    for method in ("cheap_predict8_width015","cheap_predict8_arc05_width015","cheap_predict8_locked_width015","batch_history_first8_width015"):
        for split in ("ordinary","stress"):
            baseline={r["case_id"]:r for r in rows if r["problem"]==4 and r["method"]=="range_width015" and r["split"]==split}
    ...
    for case,methods in selected.items():
        records={name:diagnose(name,paths,case) for name in ("range_width015",*methods)}
        comparisons=[]
        for method in methods:
            delta={kind:{key:records[method]["task_costs"][kind][key]-records["range_width015"]["task_costs"][kind][key]
                         for key in records[method]["task_costs"][kind]} for kind in ("survey","source")}
            comparisons.append(dict(candidate=method,baseline="range_width015",task_cost_delta=delta,
                                    total_delta_s=records[method]["total_virtual_s"]-records["range_width015"]["total_virtual_s"]))
