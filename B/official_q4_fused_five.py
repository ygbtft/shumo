"""Five authorized official Q4 practice cases of nearest/fraction30/share2."""
import argparse
...
PARAMETERS = dict(trial_radius=35., share_limit=2, share_cooldown=150.,
                  transverse_m=40., fraction=.30, steps=10, pause_limit=16,
                  dispatch='nearest')
...
def construct(problem,client,changes):
    ...
    policy=original.construct(4,client,{'fraction':.30,'share_limit':2})
...
'''.replace('PARAMETERS_LITERAL', repr(PARAMETERS))
    ...
    schedule = [dict(order=i, problem=4, phase='fused-five', block=i,
        method='range_grid21_29', setting='nearest35_fraction30_share2',
    ...
    config = dict(created_at=datetime.now().astimezone().isoformat(), schedule=schedule,
        ...
        official_formal_runs=0, mock_executions=0, observed_fused_results=0,
...
'''
    (out/'package/B/check_fused_config.py').write_text(check)
    checked = subprocess.run([sys.executable,'-B','check_fused_config.py'],cwd=out/'package/B',capture_output=True,text=True,check=True)
    ...
    runner = runner.replace('BSensitivity-20260913','BQ4FusedFive-20260913').replace('OfficialSensitivity-20260913','OfficialQ4FusedFive-20260913')
    ...
    (out/'PLAN.md').write_text('Q4官方演练5局：最近邻任务排序、35米门限、fraction=0.30、share_limit=2，其余参数保留默认。\n'
...
def execute(out):
    ...
    with (out/'batch.lock').open('a') as lock:
        ...
        check=runner.ap.guest(r'C:\Python314-arm64\python.exe -B C:\BQ4FusedFive-20260913\B\check_fused_config.py')
        ...
        for item in config['schedule']:
            ...
            print('Q4_FUSED_PROGRESS',len(runs),5,json.dumps(dict(case=row['case_code'],cleared=row['cleared'],sources=row['sources'],average_time=row['seconds_per_source'],program_runtime_s=program_time(run))),flush=True)
        ...
        summary=[summarize('历史完整策略35米',baseline),summarize('历史最近邻35米',nearest),summarize('本次最近邻35米＋fraction0.30＋share_limit2',runs)]
        for r in summary:
        ...
        runner.collect(out)
        post=runner.ap.guest(r'C:\Python314-arm64\python.exe -B "\\Mac\Home\Downloads\OfficialQ4FusedFive-20260913\verify.py" "C:\BQ4FusedFive-20260913" "\\Mac\Home\Downloads\OfficialQ4FusedFive-20260913\runtime_manifest.json"')
