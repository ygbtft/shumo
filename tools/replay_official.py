...
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / 'algorithms/q34'))
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / 'interfaces'))
import strategy as candidates
...
def replay(problem, trace):
    ...
    # 同时核对新版运行入口的日志统计；输入仍是已保存的官方响应。
    folder = create_output(ROOT / 'outputs/replay_logs', 'q' + str(problem))
    journal = RequestLog(folder / 'requests.jsonl')
    cli = Client(transport, robot_id='recorded-replay-only', transcript=journal)
    policy = candidates.construct(problem, cli)
    started = time.perf_counter()
    policy.run()
    assert index == len(records)
    metrics = journal.summary(policy, cli, started, completed=True)
    official_ms = records[-1]['response']['real_timestamp_ms'] - records[0]['response']['real_timestamp_ms']
    assert metrics['program_runtime_s'] == official_ms / 1000
    assert metrics['official_calls'] == len(records)
    assert metrics['average_localization_clear_s'] == cli.virtual_s / len(policy.cleared)
    return dict(problem=problem, commands=index, max_position_delta_m=max_delta,
                ...
                program_runtime_s=metrics['program_runtime_s'],
