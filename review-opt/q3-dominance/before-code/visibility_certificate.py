...
def directional_witness(points,g,receive_radius=1000.):
    ...
    # Tolerances make witness search conservative: None means no witness found, not coverage.
    delta=np.asarray(points)-g
    distance=np.linalg.norm(delta,axis=1)
    if np.any(distance<=1e-8):
        return None
    delta=delta[distance<=receive_radius+1e-7]
    if not len(delta):
        return {"position":np.asarray(g).tolist(),"direction_deg":0.,"reason":"no_station_in_range"}
...
def rectangle_certificate(points,arena_radius=1800.,receive_radius=1000.,max_depth=16,save_cells=True,max_cells=200000):
    ...
    stack=[(0.,0.,float(arena_radius),0)]
    cache={}
    cells=[]
    visited=0
    deepest=0
    ...
    min_hull=math.inf
    while stack:
        # h is half the side length. Outside-disk leaves need no certificate.
        x,y,h,depth=stack.pop()
        visited+=1
        deepest=max(deepest,depth)
        c=np.array([x,y])
        nearest=np.maximum(np.abs(c)-h,0.)
        if float(nearest@nearest)>arena_radius**2+1e-6:
            continue
        farthest=np.linalg.norm(np.abs(p-c)+h,axis=1)
        ids=tuple(np.flatnonzero(farthest<=receive_radius-1e-5).tolist())
        if ids not in cache:
            try:
                cache[ids]=ConvexHull(p[list(ids)]).equations if len(ids)>=3 else None
            except QhullError:
                cache[ids]=None
        equations=cache[ids]
        if equations is not None:
            # Unit outward hull normals: h*||n||_1 is the square support function.
            # Its worst value proves containment of the continuous cell, not just its center.
            ...
            if margins.min()>1e-5:
                ...
                cells.append([x,y,h,list(ids)])
                continue
        # max_cells is checked at unresolved cells, not a strict memory/visit limit.
