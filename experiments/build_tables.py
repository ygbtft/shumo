...
共纳入927个不同的官方演练案例：消融160局、参数宽扫500局、联合扰动240局、Q3融合方案10局、Q3中止复验7局、Q4融合参数候选5局、Q4第二次正式方案同配置复测5局。另有4局接入校验未计入结果表。表1与表3重复展示同一组Q3基准，表2与表4重复展示同一组Q4基准，不能把四表行数乘以局数当成独立总样本量。统计读取官方原始记录，不调用模拟器。
...
四表的程序运行时间统一采用官方进入与退出响应的时间戳差；Q3 baseline 均值为0.7217秒。逐局审计另存客户端`robot.wall_s`用于核对，不将该墙钟指标混入表格均值或偏移计算。

原始来源包括：

- `data/experiments/2026-09-13_official-ablation`：160局正式纳入的消融演练及2局接入。
- `data/experiments/2026-09-13_official-sensitivity`：500局参数宽扫、240局联合扰动及2局接入；实验分析记录见`PLAN.md`、`analysis_plan.json`、`REPORT.md`、`robustness.json`。
