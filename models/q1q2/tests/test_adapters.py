...
import pytest
from mock.strategy import Action, Observation
from models.q1q2.adapters import (ErrorMode, ObservationRecord, observations_to_bearings,
                                  observation_record, read_jsonl, read_measurements)
from models.q1q2.run import jsonable, write_json
from models.q1q2.geometry import NumericPolicy
...
def test_mock_public_observation_wrapper_and_no_truth_fields():
    obs = Observation(Action('/measure',(10,20),1), {'accepted':True,'measure_result':'direction','svd_deg':30,
                      'source_position':(99,99),'radius':1234},(10,20),1,5.,100.,frozenset(),1)
    converted = observations_to_bearings([observation_record(obs,session_id='one')],1,ErrorMode.THEORETICAL_1_DEG)
    assert converted[0].position == (10,20)
    assert converted[0].bearing_deg == 30
    assert 'source_position' not in jsonable(converted[0])
    assert 'radius' not in jsonable(converted[0])
