...
针对问题2，利用首次正反馈对固定未知接收半径进行条件化，导出完整保收域，并将未知第二读数下的最坏直径转化为共同反馈源位置对的最大距离。标准首测场景的保收域精确约化为四圆盘交；经50/25米网格、边界扫描与多起点细化，推荐第二点 $(843.035666,545.527004)$ 米，其连续最坏直径认证区间为 $[110.969134,110.970102]$ 米，移动并检测需 205.829 秒。相较固定点 $(750,400)$ 米，最坏直径至少缩小 36.301 米，但仍不能保证任何反馈后均可用一次20米统一落点清除。另证明移动预算仅10米时，最坏剩余歧义至少1000米。推荐点为当前分辨率下的最佳数值候选，固定坐标认证不证明外层全局最优。
...
独立几何与清除基准按外包安全合同（"绝不删除真源"）279项全部通过；若改用更严的精确后验比较则为277/279，另2项是保守多留不可行候选的位置更新差异，未发现删真源或错误保证清除的安全反例。官方模拟器演练（practice）统计库记录：问题3所列5局全部清除，共63源，源加权用时约249.4秒/源；问题4所列6局全部清除，共80源，约449.1秒/源，其中一局计时受会话复用影响；完整历史另含一局早期问题4失败。本模型的任务适配增量在于，将接收信息的条件化、定位歧义和清除动作的可行性连成可复核的论证链，实验结论限于所列配置与样本。
...
问题2采用“解析候选检查—全域网格—源点对评分—多起点局部细化—统一精度复评”的求解方案。对每个站点，使用满足式（16）的公共源样本枚举式（26）的合法点对，得到 $\widehat J(q)$，再从最长点对出发细化源位置。外层在有物理依据的候选域包围盒内铺设50米、25米网格，补入候选边界点，并从多个分散低值起点局部细化。源采样以角度和径向参数组织，基础三级为 $9\times25$、$17\times49$、$33\times97$，另外保留合法边界事件及近场附近样本。这些为基础分辨率；标准场景已完成的全域扫描与加密记录见第5.3节。

终选点用相同细级源样本与局部细化规则复评，将新增合法见证点合并后供各候选公平比较。默认数值并列阈值取0.1米与终选比较最大细化变化二者的较大值，进入该带的候选再按移动距离择优。该带只表示当前数值分辨率下效果相当；第5.3节外层定稿实验另将并列带置零，以报告主目标的数值最小候选。
...
以下结果具有闭式依据或已有独立核验，长度均以米计；第二点优化的数值结果另见第5.3节。
...
该表按**外包安全合同**判定：验收要求是"绝不删除真源"，而非"后验精确最小"。按此合同全部279项通过。若改用更严的精确后验成员比较，则问题3单次无信号后有2项不一致：检测点为原点时，候选位置 $(999.999,0)$、$(1000,0)$ 按闭接收盘应与无信号不相容，程序却保留了这两个候选。**该差异仍然存在**，性质是外包偏松、未充分利用负信息，方向上偏保守：多留不可行候选，不会删真源，也不会造成错误的保证清除。两种口径的记录分别为精确后验比较277/279与安全合同279/279；本文以安全合同为验收依据，并保留上述差异说明。所检安全性质未见反例，但这只支持测试范围内的包含与清除判断，不证明任意输入安全。
...
| 标准场景第二检测点推荐 | 全域网格、边界扫描、多起点细化及两点同精度认证 | 推荐坐标的连续直径已认证；外层仅为数值候选 |
...
| 独立benchmark 安全合同279/279通过 | 第9.5节独立真值比较 | 精确后验口径为277/279,另2项为保守外包差异;所检安全性质未见反例 |
| Q3约249.4、Q4约449.1秒/源 | 官方演练统计汇总 | 所列practice样本，Q4含计时异常局 |

## 10 相关工作与模型评价

有界示向误差的角锥逆像、集合求交与最坏直径评价源于既有集员定位研究[R1–R2]。最小覆盖圆与半径—直径关系采用经典计算几何结果[R14–R15]。因此，本文对问题1、2的增量定位为**三组题目适配贡献**，而非新方法的学术首创：①首次正反馈对共享未知接收半径的条件化，及标准首测场景下保收域精确约化为四圆盘交；②位置可行集与统一清除动作的耦合，即以最小覆盖半径不超过20米把定位精度直接翻译成一次落点的可行性；③共同反馈源位置对的重写，及移动预算受限时的不可辨识下界。区间认证使上述具体数值结论更可信，但区间分析本身不是本文发明。需要强调，**命题成立与文献原创性是两个独立维度**：写成定理只说明在给定模型下有完整证明，不自动意味着首次提出。
...
主动测向中计入移动、检测成本及定位阈值已有研究[R7，R9]。本文对问题3、4只主张**两项主要贡献**。第一项是**任意发射朝向下的连续几何发现证书**：把"未知朝向仍必被某站接收"的要求转化为局部可接收站的凸包包含条件，配合四叉分区构造覆盖整个源圆盘的连续证书，并以独立整数几何复核所选二十一站；七站全向方案是该证书在朝向已知情形下的基线特例，其连续覆盖半径992.689147米由解析验证，不作为独立的理论贡献。第二项是**成功锚点约束下的成对探测安全截断**：利用一次成功反馈提供的锚点构造成对探点，在无需识别发射朝向的条件下证明远侧候选至少在一处可接收，据此把完整双负反馈转化为保持真源的半平面截断；不主张"任意探测均有效"或"无信号即可缩半"。

其余工作作为上述两项的**支撑**而非并列贡献：联合任务重排是效率启发式，完成性由覆盖排尽与不可重置的有限服务预算支撑；独立几何基准按外包安全合同验收，支持所测范围内的实现安全性，不证明任意输入安全；清除门限的时间—失手折中是所测场景下的经验权衡。对文献[R2]、[R3]、[R9]仅引用文献审阅已核实的摘要层面事实，不引用未核对的定理细节；有限检索只能缩小创新主张，不能证明没有相同先例，故不据此声称全球首创。
...
独立benchmark按外包安全合同279项全部通过（精确后验口径为277/279，另2项为保守外包差异），所检安全性质未见反例。理论部分证明指定条件下的覆盖、截断与停止依据，数值调度部分给出已执行的启发式策略。官方模拟器practice统计支持所列Q3五局63源、Q4六局80源全部清除，源加权用时分别约249.4和449.1秒/源；完整历史另有一局早期失败，Q4时间汇总含一局受污染计时。故当前结论是已获证几何框架下的有限演练效果，不是任意场景的零失败或最短时间承诺。
...
| [Q2外层推荐与统一复评](q1q2/benchmarks/q2_outer/report.json)、[复现说明](q1q2/benchmarks/q2_outer/README.md) | 标准场景推荐点与固定点的0.001米认证、条件覆盖及移动代价；不认证连续外层最优 |
| [Q3/Q4独立基准](../B/benchmarks_q34/report.json) | 安全合同下279/279通过;精确后验口径277/279与2项保守多留候选的记录;不作为整局全清率 |
...
Q3/Q4实现以已合并的协作分支 `NTJ_B` 为基础，经审计与修复形成当前候选；本稿统一四问的模型、结果与证据口径。Q3/Q4部分读取既有报告，未重新运行其benchmark、正式测试或查询实时统计库；Q2标准场景的外层搜索与固定坐标认证已按第5.3节新增脚本重跑。

## 附录B 定稿前待补清单

- **TODO-1｜已完成：问题2标准场景最终推荐方案。**第5.3节表2（续）已给出外层搜索所得推荐点、与固定点的同精度认证、条件覆盖外包、清除判定、移动时间及网格/容差细化记录；脚本与原始报告见 `q1q2/benchmarks/q2_outer/`。结论为当前分辨率下的最佳候选，未声称连续外层全局最优；其他首测场景不在本次定稿范围。
