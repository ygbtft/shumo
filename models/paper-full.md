...
针对有限接收距离与有界示向误差下的干扰源定位清除问题，建立由位置可行集、覆盖证书和序贯行动组成的模型。问题1将前向角锥交集转化为半平面交，计算定位区域直径，并证明半径为直径一半的覆盖圆若存在，其圆心必为最远点对中点；构造合法三站观测产生的等边三角形反例，说明直径圆并非总能覆盖。利用最小覆盖圆与Jung界，将统一20米清除落点的存在性转化为覆盖半径不超过20米。

问题2根据首次正反馈对同源固定未知接收半径进行条件化，导出第二点的完整保收域，并将未知读数下的最坏定位直径表示为共同反馈源位置对的最大距离。在标准首测场景中，保收域约化为四圆盘交；构造同射线候选源对，证明移动预算仅10米时仍可能保留至少1000米的位置歧义。有限网格和局部细化仅用于求取数值候选，不作为连续全局最优证明。

问题3在上述清除判据基础上建立“发现—定位—清除”闭环。原点与半径1140米的六个等角站构成七站布局，其全域最近站距离上界为992.689147米，小于最小接收半径1000米。问题4将任意180度发射朝向的发现条件转化为局部可达站点的凸包包含条件，对二十一站实际整数坐标给出连续单元覆盖证书，并利用成对探测的双负反馈安全收缩位置外包。两问均以剩余扫描任务和已发现源任务联合排序，采用有限定位预算与光学网格兜底；排序、选点代理及试探清除属于启发式。

据官方模拟器统计库的演练（practice）记录，Q3所列5局全部清除，共63源，源加权用时约249.4秒/源；Q4所列6局全部清除，共80源，约449.1秒/源，其中一局存在会话复用导致的计时污染。完整入场记录另含一局早期Q4失败，故历史记录为12局中11局全清。上述成绩不含正式测试，不构成任意场景全清或最短时间保证。

**关键词：** 集员定位；最小覆盖圆；覆盖证书；定向源；耦合调度
...
| 为可解性简化 | 问题2使用连续读数模型及浮点网格、局部细化求数值候选 | 解析定义保持连续，有限搜索不提供全局最优保证 |
...
| 为可解性简化 | 问题3用区域面积先验和协方差代理排序，问题4用有限成对探测及预测任务点 | 只影响效率，不将这些先验视为题面分布或位置包含依据 |
...
原Q1/Q2草稿所依据的圆覆盖基准曾记录一个近共线强制外接圆的数值失败。后续汇总已有修复及新一轮核验记录，但本次整合未重新运行该基准，因此保留其历史风险说明，仅引用本节已独立核验的具体锚点，不将旧失败断言为当前仍未修复，也不宣称所有退化输入均已认证正确。
...
保证清除之外，策略允许在包含半径尚大于20米时向中心进行一次试探清除。当前入口Q3门限为50米，Q4为35米；一次试探失败后继续定位或兜底，不把“尝试过”记为“已清除”，也不据此删除整个频道。试探门限是时间与失败费用的经验权衡，不能写成50米或35米内必然清除。

当有限次补充测向仍不足以达到清除条件时，以 $U_j$ 在首测方向坐标系中的外接矩形为搜索范围，划分边长至多28米的小矩形，依次访问格心并执行光学清除。每个小格中任一点到格心的距离至多
...
固定站点的距离平方是凸函数，方块内最大值可由角点控制；站点凸包包含四角就包含整个方块。因此式（47）使每个块内源位置满足式（46）。再核验叶块不重叠地覆盖整个根方形、仅跳过与源圆不相交的块，并逐一核对实际执行站点与获证坐标，才得到全域证书。单独通过若干叶块检查不代表覆盖完整。
...
若 $l$ 太小则转入兜底；否则两点为 $S+l u\pm wv$，$v$ 为垂直于 $u$ 的单位向量。该横向限幅满足定理的斜率条件，0.15和40米用于控制前进程度与横向移动，属于经验参数。按距当前位置较近者先测，成功后更新角锥并结束本轮；只有两点均无信号才能执行截断，不能在只完成一半时提前使用双负反馈结论。
...
问题1、2针对题设的主要增量体现在四个方面：利用首次正反馈对共享未知接收半径条件化，导出保收策略类内完整候选域及标准场景四盘约化；将未知第二读数下的最坏直径转为共同反馈点对最大距离；把定位集合与20米清除落点联系起来；通过同射线候选源对证明移动预算下的短基线歧义下界。这些是本题的建模与推导结果，不据有限文献检索声称全球首创。$\kappa,\eta$ 用于区分最佳圆心与直径中点的风险，基础仍是经典覆盖几何。
...
问题3、4在既有集员定位与主动观测思想上，将“至少一次发现”与“达到统一清除条件”分开处理：前者由全向距离覆盖或任意方向凸包证书承担，后者由位置外包、合法负反馈与有限光学搜索承担。耦合调度在二者之间分配移动和检测资源。相关文献[1-2,7-8]支持集合定位、主动选点与时间成本的基本思想，不能直接替代本题有限接收半径、定向半平面和未知源数停止条件的推导。

实现来源也应与论文推导区分。Q1/Q2使用已有独立模型草稿，Q3/Q4策略以合并的协作分支 `NTJ_B` 为基座，经只读审计、独立几何复核和后续外围修复形成当前主候选。本文承担四问统一表述及证据衔接，不把既有策略代码改称本次从零开发，也不据文献检索范围声称全球首创。

模型的优势在于关键删除和停止动作具有可追踪前提：20米落点检查对应位置包含，距离删测对应最大接收半径，未知频道结束对应完整覆盖或16源上限。其局限也直接影响结论：工程定位仍有浮点外包风险；试探清除允许失败；时间代理未精确优化全部未来反馈；演练样本少且包含计时异常；Q2代表场景的完整数值推荐表仍待汇总。上述缺项不能由Q3/Q4的全清实验替代。

## 11 结论

四问可统一为“保留相容位置—获得足够观测—执行有依据的清除动作”。问题1给出定位区域、直径与圆覆盖判据；问题2利用首次接收信息建立稳健第二点选择，并证明短基线可能保留大尺度歧义；问题3用七站全向覆盖和时间导向服务实现多源闭环；问题4以二十一站任意方向覆盖及成对负反馈处理未知发射朝向。

理论部分证明指定条件下的覆盖、截断与停止依据，数值调度部分给出已执行的启发式策略。官方模拟器practice统计支持所列Q3五局63源、Q4六局80源全部清除，源加权用时分别约249.4和449.1秒/源；完整历史另有一局早期失败，Q4时间汇总含一局受污染计时。故当前结论是已获证几何框架下的有限演练效果，不是任意场景的零失败或最短时间承诺。
...
正文延续原Q1/Q2草稿的顺序编码[1]—[8]。文献审阅表中的R编号是另一套检索索引，不能直接按数字对应：正文[1]、[2]、[3]、[4]、[5]、[6]、[7]、[8]分别对应审阅表R1、R2、R14、R15、R5、R6、R7、R9。审阅表R3、R4、R8未在原正文列为正式参考文献，本次不为凑齐R1—R9新增未使用条目。

| 材料 | 支持内容及限制 |
|---|---|
| [Q1/Q2原稿](q1q2/paper-q1q2.md)与[文献审阅](q1q2/reviews/literature-review.md) | 第1—5节基础及参考文献编号；保留原稿未完成的Q2数值汇总说明 |
| [Q34权威汇总](../Q34-SUMMARY.md) | practice统计、异常局、修订顺序；官方成绩以此转录 |
| [覆盖搜索审计](q1q2/peer-audit/q34/audit-coverage-search.md)与[定向源审计](q1q2/peer-audit/q34/audit-directional.md) | 七站、二十一站及负反馈推导；审计发现的历史辅助缺陷应结合汇总中的修复状态阅读 |
| [路线时间审计](q1q2/peer-audit/q34/audit-route-time.md)与[总体审计](q1q2/peer-audit/q34/audit-overall.md) | 真实费用与代理目标区分、主配置与历史分支区分 |
| [当前候选入口](../B/bounded_candidates.py)与[二十一站坐标](../B/layouts/grid21_29.json) | 当前50/35米门限与实际运行站集合；不证明归档官方演练已使用最新门限 |
| [成对探测](../B/bounded_width_policy.py)、[耦合调度](../B/coupled_dispatch_policy.py)、[定位外包](../B/geometry.py) | 公式与现行代码衔接；代码存在不等于形式化数值认证 |
| [独立HTTP合成验证](q1q2/peer-audit/q34/scale-ours-mock.md) | 2400次合成执行的统计、配对依赖与服务器适配说明 |
| [清除门限实验](../B/CLEAR_GATE_TRADEOFF.md) | 本地参数验证；不与官方practice时间合并计分 |

本文在独立Q3/Q4建模文档尚未落盘时，以已有代码、审计和权威汇总完成整合；不依赖并行建模文档的未完成内容。
