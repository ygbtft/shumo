...
try{
 ...
 elseif($cfg.action -eq 'navigate'){
  ...
  if($script:active.Current.Name -ne '问题4正式测试'){throw 'Failed to reach Q3 formal page'}
  $result.status='q4-formal-page'
 }elseif($cfg.action -eq 'start'){
  if($script:active.Current.Name -ne '问题4正式测试' -or '已用1次，剩余2次（共3次）' -notin $script:names -or '可以开始' -notin $script:names){throw 'Formal page/quota changed'}
  if(Test-Path 'C:\Q4NearestFormal-20260913\ONE_FORMAL_ATTEMPT.json'){throw 'Robot attempt already exists'}
  if(Test-Path 'C:\Q4NearestFormal-20260913\FORMAL_CLIENT_STARTED.json'){throw 'Robot already started'}
  & 'C:\Python314-arm64\python.exe' -B 'C:\Q4NearestFormal-20260913\verify_package.py' 'C:\Q4NearestFormal-20260913' | Out-File -Encoding UTF8 "$JobDir\package-verification.txt"
  if($LASTEXITCODE -ne 0){throw 'Package validation failed'}
  $marker=[System.IO.File]::Open('C:\Q4NearestFormal-20260913\GUI_FORMAL_START_ONCE.json',[System.IO.FileMode]::CreateNew,[System.IO.FileAccess]::Write,[System.IO.FileShare]::None)
  try{$bytes=[System.Text.Encoding]::UTF8.GetBytes((@{problem=4;prior_used=1;at=[DateTime]::UtcNow.ToString('o')}|ConvertTo-Json));$marker.Write($bytes,0,$bytes.Length)}finally{$marker.Dispose()}
  ...
  if($script:active.Current.Name -ne '问题4正式测试' -or -not @($script:els|Where-Object {$_.Current.Name -eq '即将开始问题4正式测试。当前剩余2次机会，是否继续？'}).Count){throw 'Wrong formal confirmation'}
  if(-not (Test-Path 'C:\Q4NearestFormal-20260913\GUI_FORMAL_START_ONCE.json')){throw 'Missing authorized start marker'}
  $marker=[System.IO.File]::Open('C:\Q4NearestFormal-20260913\GUI_FORMAL_CONFIRM_ONCE.json',[System.IO.FileMode]::CreateNew,[System.IO.FileAccess]::Write,[System.IO.FileShare]::None)
  try{$bytes=[System.Text.Encoding]::UTF8.GetBytes((@{problem=4;formal_index=2;at=[DateTime]::UtcNow.ToString('o')}|ConvertTo-Json));$marker.Write($bytes,0,$bytes.Length)}finally{$marker.Dispose()}
  InvokeExact '继续'
  ...
  if(-not (Test-Path 'C:\Q4NearestFormal-20260913\GUI_FORMAL_CONFIRM_ONCE.json')){throw 'Missing prior confirmation marker'}
  $marker=[System.IO.File]::Open('C:\Q4NearestFormal-20260913\GUI_FORMAL_FINAL_ONCE.json',[System.IO.FileMode]::CreateNew,[System.IO.FileAccess]::Write,[System.IO.FileShare]::None)
  try{$bytes=[System.Text.Encoding]::UTF8.GetBytes((@{problem=4;formal_index=2;at=[DateTime]::UtcNow.ToString('o')}|ConvertTo-Json));$marker.Write($bytes,0,$bytes.Length)}finally{$marker.Dispose()}
  InvokeExact '确认开始'
  Start-Sleep -Seconds 2
  ...
  if(-not (Test-Path 'C:\Q4NearestFormal-20260913\GUI_FORMAL_FINAL_ONCE.json')){throw 'Missing final start marker'}
  $mutex.ReleaseMutex();$held=$false
  $result.robot_started=$true
  & 'C:\Q4NearestFormal-20260913\start_q4_nearest_formal.ps1' -RobotId ([string]$cfg.robot_id) -ExpectedCase ([string]$cfg.expected_case) *> "$JobDir\robot-launch.log"
  ...
  if($script:active.Current.Name -ne '问题4正式测试' -or $cfg.expected_case -ne '5FS5-MYSH-FKHB-PRX4' -or $cfg.expected_case -notin $script:names -or '测试已结束' -notin $script:names){throw 'Wrong completed formal case'}
