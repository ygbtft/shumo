...
function Snapshot($label){
 ...
 foreach($e in $script:els){$c=$e.Current;$b=$c.BoundingRectangle;$rows.Add(@{text=$c.Name;class=$c.ClassName;id=$c.AutomationId;type=$(if($null -ne $c.ControlType){$c.ControlType.ProgrammaticName}else{''});enabled=$c.IsEnabled;offscreen=$c.IsOffscreen;rect=@($b.X,$b.Y,$b.Width,$b.Height)})}
...
try{
 if($cfg.policy -ne 'formal-q4-once' -or $cfg.problem -ne 4 -or [System.Diagnostics.Process]::GetCurrentProcess().SessionId -ne 2){throw 'Formal Q4 authorization scope mismatch'}
 ...
 elseif($cfg.action -eq 'navigate'){
  ...
  if($script:active.Current.Name -ne '问题4正式测试'){InvokeExact '问题4正式测试'}
  Snapshot 'q4-formal-page'
  if($script:active.Current.Name -ne '问题4正式测试'){throw 'Failed to reach Q4 formal page'}
  $result.status='q4-formal-page'
 }elseif($cfg.action -eq 'start'){
  if($script:active.Current.Name -ne '问题4正式测试' -or '已用2次，剩余1次（共3次）' -notin $script:names -or '可以开始' -notin $script:names){throw 'Formal page/quota changed'}
  if(Test-Path 'C:\Q4NearestFormalRepeat-20260913\ONE_FORMAL_ATTEMPT.json'){throw 'Robot attempt already exists'}
  if(Test-Path 'C:\Q4NearestFormalRepeat-20260913\FORMAL_CLIENT_STARTED.json'){throw 'Robot already started'}
  & 'C:\Python314-arm64\python.exe' -B 'C:\Q4NearestFormalRepeat-20260913\verify_package.py' 'C:\Q4NearestFormalRepeat-20260913' | Out-File -Encoding UTF8 "$JobDir\package-verification.txt"
  if($LASTEXITCODE -ne 0){throw 'Package validation failed'}
  $marker=[System.IO.File]::Open('C:\Q4NearestFormalRepeat-20260913\GUI_FORMAL_START_ONCE.json',[System.IO.FileMode]::CreateNew,[System.IO.FileAccess]::Write,[System.IO.FileShare]::None)
  try{$bytes=[System.Text.Encoding]::UTF8.GetBytes((@{problem=4;prior_used=2;at=[DateTime]::UtcNow.ToString('o')}|ConvertTo-Json));$marker.Write($bytes,0,$bytes.Length)}finally{$marker.Dispose()}
  InvokeExact '开始问题4正式测试'
  Snapshot 'after-single-start-click'
  $result.status='single-start-clicked'
 }elseif($cfg.action -eq 'confirm-start'){
  if($script:active.Current.Name -ne '问题4正式测试' -or -not @($script:els|Where-Object {$_.Current.Name -eq '即将开始问题4正式测试。当前剩余1次机会，是否继续？'}).Count){throw 'Wrong formal confirmation'}
  if(-not (Test-Path 'C:\Q4NearestFormalRepeat-20260913\GUI_FORMAL_START_ONCE.json')){throw 'Missing authorized start marker'}
  $marker=[System.IO.File]::Open('C:\Q4NearestFormalRepeat-20260913\GUI_FORMAL_CONFIRM_ONCE.json',[System.IO.FileMode]::CreateNew,[System.IO.FileAccess]::Write,[System.IO.FileShare]::None)
  try{$bytes=[System.Text.Encoding]::UTF8.GetBytes((@{problem=4;formal_index=3;at=[DateTime]::UtcNow.ToString('o')}|ConvertTo-Json));$marker.Write($bytes,0,$bytes.Length)}finally{$marker.Dispose()}
  InvokeExact '继续'
  Start-Sleep -Seconds 2
  Snapshot 'after-single-confirmation'
  $result.status='single-start-confirmed'
 }elseif($cfg.action -eq 'confirm-final'){
  if($script:active.Current.Name -ne '问题4正式测试' -or -not @($script:els|Where-Object {$_.Current.Name -eq '启动成功后将占用问题4的一次正式测试机会。确定开始吗？'}).Count){throw 'Wrong final confirmation'}
  if(-not (Test-Path 'C:\Q4NearestFormalRepeat-20260913\GUI_FORMAL_CONFIRM_ONCE.json')){throw 'Missing prior confirmation marker'}
  $marker=[System.IO.File]::Open('C:\Q4NearestFormalRepeat-20260913\GUI_FORMAL_FINAL_ONCE.json',[System.IO.FileMode]::CreateNew,[System.IO.FileAccess]::Write,[System.IO.FileShare]::None)
  try{$bytes=[System.Text.Encoding]::UTF8.GetBytes((@{problem=4;formal_index=3;at=[DateTime]::UtcNow.ToString('o')}|ConvertTo-Json));$marker.Write($bytes,0,$bytes.Length)}finally{$marker.Dispose()}
  InvokeExact '确认开始'
  Start-Sleep -Seconds 2
  ...
  if($script:active.Current.Name -ne '问题4正式测试'){throw 'Not Q4 formal'}
  if($cfg.expected_case -notmatch '^[A-Z0-9]{4}(-[A-Z0-9]{4}){3}$' -or $cfg.expected_case -notin $script:names){throw 'Expected case mismatch'}
  if('等待机器狗进入' -notin $script:names -or '尚未进入' -notin $script:names){throw 'Case is not waiting for first entry'}
  if(-not (Test-Path 'C:\Q4NearestFormalRepeat-20260913\GUI_FORMAL_FINAL_ONCE.json')){throw 'Missing final start marker'}
  $mutex.ReleaseMutex();$held=$false
  $result.robot_started=$true
  & 'C:\Q4NearestFormalRepeat-20260913\start_q4_nearest_formal.ps1' -RobotId ([string]$cfg.robot_id) -ExpectedCase ([string]$cfg.expected_case) *> "$JobDir\robot-launch.log"
  if($LASTEXITCODE -ne 0){throw 'Formal robot launch failed; inspect logs; do not retry'}
  Start-Sleep -Seconds 2
  Snapshot 'formal-completed'
  if('测试已结束' -notin $script:names){throw 'Robot returned but official UI has not ended'}
  $result.case_code=$cfg.expected_case;$result.status='formal-robot-completed'
 }elseif($cfg.action -eq 'view-upload'){
  if($script:active.Current.Name -ne '问题4正式测试' -or $cfg.expected_case -notmatch '^[A-Z0-9]{4}(-[A-Z0-9]{4}){3}$' -or $cfg.expected_case -notin $script:names -or '测试已结束' -notin $script:names){throw 'Wrong completed formal case'}
  if(-not @($script:els|Where-Object {$_.Current.Name -eq '问题4正式测试完成'}).Count){throw 'Missing completion dialog'}
  InvokeExact '确认'
  Snapshot 'completion-dismissed'
  InvokeExact '返回问题4正式测试'
