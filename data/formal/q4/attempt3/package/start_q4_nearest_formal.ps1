...
& $python -B "$PSScriptRoot\B\run_q4_nearest_formal.py" --check-config
...
function ReadFormalCase {
 ...
 if($active.Current.Name -notmatch '^问题\s*4\s*正式测试$'){throw 'Open the Q4 FORMAL session first; no robot request sent'}
 ...
 if($text -notmatch '问题\s*4\s*正式\s*测试' -or $text -notmatch '等待机器狗进入' -or $text -notmatch '尚未进入' -or $text -match '测试已结束'){throw 'Q4 formal case must be waiting, not already entered or ended'}
...
if($CheckOnly){Write-Output "READY: Q4 formal case $case; nearest task order; gate 35 m, default parameters; no requests sent";return}
...
try {
 ...
 try{
  $record=@{case_code=$case;problem=4;variant='range_grid21_29_nearest35';robot_id=$RobotId;started_utc=[DateTime]::UtcNow.ToString('o')}
 ...
 & $python -B run_q4_nearest_formal.py --robot-id $RobotId --case-code $case --confirm-formal
 $code=$LASTEXITCODE
 $archive="\\Mac\Home\Downloads\Q4NearestFormalRepeat-20260913\results\$case"
 ...
 $official=@(Get-ChildItem 'C:\Jammers\JammersSimulatorData\behavior-logs' -File|Where-Object {$_.Name -like "formal-p4-*-$case*"})
