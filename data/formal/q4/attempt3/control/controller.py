...
runner = ap.Runner(SimpleNamespace(problem=4, robot_id='202623001141', execute=False, reject_formal=False, runs=1))
try:
    while True:
        try:
            runner.setup()
            ...
            if runner.lock:
                runner.lock.close()
            if datetime.now().astimezone() >= datetime.fromisoformat('2026-09-13T17:00:00+08:00'):
                raise RuntimeError('Formal start preparation cutoff reached')
            print('WAITING_FOR_SHARED_CONTROLLER', flush=True)
            time.sleep(20)
