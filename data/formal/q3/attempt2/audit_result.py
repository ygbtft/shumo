...
def main():
    ...
    meta = read(case_dir / f'formal-p3-2-{CASE}.result.json')
    upload, = read(ROOT / 'formal-upload-snapshot/rows.json')['rows']
    jlog = case_dir / f'formal-p3-2-{CASE}.jlog'
    ...
    assert meta['formal_index'] == upload['formal_index'] == 2
    ...
    assert CASE in names and '已用2次，剩余1次（共3次）' in names and '已上传' in names
    result = dict(case_code=CASE, formal_index=2, cleared=counts['cleared'],
