...
def critical_cover_radius(points,arena_radius=1800.):
    """Analytic critical-point evaluation of disk coverage, with numeric margin.

    Interior maxima of nearest-site distance occur at Voronoi vertices.
    On the disk boundary, maxima occur at a Voronoi switch or a stationary
    distance from the active site. Enumerating extra bisector-circle crossings
    is harmless. Near-degenerate Qhull failures use the outer-cell upper bound.
    """
    ...
    candidates=[arena_radius*np.array([[1.,0.],[-1.,0.],[0.,1.],[0.,-1.]])]
    norms=np.linalg.norm(p,axis=1);nonzero=norms>1e-10
    if nonzero.any():candidates.append(-arena_radius*p[nonzero]/norms[nonzero,None])
    i,j=np.triu_indices(len(p),1)
    if len(i):
        delta=p[j]-p[i];length=np.linalg.norm(delta,axis=1);normal=delta/length[:,None]
        distance=(np.sum(p[j]**2,axis=1)-np.sum(p[i]**2,axis=1))/(2*length)
        inside=abs(distance)<=arena_radius+1e-7
        normal=normal[inside];distance=np.clip(distance[inside],-arena_radius,arena_radius)
        foot=distance[:,None]*normal;tangent=np.column_stack((-normal[:,1],normal[:,0]))
        shift=np.sqrt(np.maximum(0,arena_radius**2-distance**2))[:,None]*tangent
        candidates.extend([foot+shift,foot-shift])
    if len(p)>=3:
        centered=p-p.mean(axis=0)
        singular=np.linalg.svd(centered,compute_uv=False)
        if singular[-1]>1e-10*max(1,singular[0]):
            try:
                vertices=Voronoi(p).vertices
                candidates.append(vertices[np.linalg.norm(vertices,axis=1)<=arena_radius+1e-7])
            except QhullError:
                return omni_certificate(p,arena_radius,sides=256)["radius_upper_m"]
        elif singular[-1]>1e-9:
            return omni_certificate(p,arena_radius,sides=256)["radius_upper_m"]
    points_to_check=np.vstack(candidates)
    radius=float(np.linalg.norm(points_to_check[:,None,:]-p[None,:,:],axis=2).min(axis=1).max())
    return radius+1e-4
