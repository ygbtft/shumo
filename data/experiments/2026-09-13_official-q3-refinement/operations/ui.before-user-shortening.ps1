...
function Elements($root) {
 # Fresh bulk property cache for this enumeration only. Live element references
 # remain available; all action-time GuardButton/Invoke checks still use Current.
 $cache=New-Object System.Windows.Automation.CacheRequest
 $cache.TreeScope=[System.Windows.Automation.TreeScope]::Element
 $cache.TreeFilter=[System.Windows.Automation.Condition]::TrueCondition
 ...
 foreach($property in @(
  ...
  [System.Windows.Automation.AutomationElement]::BoundingRectangleProperty)){$cache.Add($property)}
...
function Snapshot($label) {
 ...
 if(@($all | Where-Object {$_.Cached.IsPassword -or $_.Cached.ControlType -eq [System.Windows.Automation.ControlType]::Edit}).Count){throw 'INPUT_OR_LOGIN_PRESENT: no capture of credentials'}
 ...
 $rows=New-Object 'System.Collections.Generic.List[object]'; try { foreach($e in $script:els){$c=$e.Cached;if($c.IsPassword -or $c.ControlType -eq [System.Windows.Automation.ControlType]::Edit){throw 'INPUT_APPEARED'};$r=$c.BoundingRectangle
 $rows.Add(@{text=$c.Name;class=$c.ClassName;id=$c.AutomationId;type=$(if($null -eq $c.ControlType){'Unavailable-during-render'}else{$c.ControlType.ProgrammaticName});offscreen=$c.IsOffscreen;enabled=$c.IsEnabled;rect=@($r.X,$r.Y,$r.Width,$r.Height)})}
 } finally {
 $rows|ConvertTo-Json -Depth 6|Set-Content -Encoding UTF8 "$JobDir\$prefix.uia.json"
 Log 'SNAPSHOT' @{label=$label;file="$prefix.png";text_file="$prefix.uia.json";session=[System.Diagnostics.Process]::GetCurrentProcess().SessionId;fresh_property_cache=$true;node_count=$rows.Count}
 }
 if([APNative]::GetForegroundWindow() -ne $process.MainWindowHandle){throw 'SIMULATOR_NOT_FOREGROUND'}
 $script:main=One @($script:els|Where-Object {$_.Cached.ClassName -match '(^|\s)workspace-main($|\s)'}) 'workspace-main'
 $script:mainEls=Elements $script:main
 $active=One @($script:els|Where-Object {$_.Cached.ClassName -match '(^|\s)nav-item\s' -and $_.Cached.ClassName -match '(^|\s)active($|\s)'}) 'active navigation'
 Deny $active.Cached.Name 'active navigation'
 foreach($e in $script:mainEls){Deny ($e.Cached.Name+' '+$e.Cached.ClassName) 'main content'}
 foreach($e in $script:els){if($e.Cached.ControlType -eq [System.Windows.Automation.ControlType]::Window -or $e.Cached.ClassName -match 'modal|dialog'){Deny (($e.Cached.Name)+ ' '+ ((Elements $e|ForEach-Object {$_.Cached.Name}) -join ' ')) 'dialog'}}
 if($active.Cached.Name -ne '演练测试'){throw 'NOT_PRACTICE_NAVIGATION'}
 if(-not @($script:els|Where-Object {$_.Cached.Name -eq '服务器已连接'}).Count){throw 'NOT_CONNECTED'}
 if(-not @($script:els|Where-Object {$_.Cached.Name -eq "队号 $($cfg.robot_id)"}).Count){throw 'TEAM_MISMATCH_OR_NOT_LOGGED_IN'}
}
...
try {
 ...
 if($cfg.action -eq 'dry-run'){$script:outcome.ok=$true;$script:outcome.status='dry-run-ready'}
  ...
  $arguments=@('-B','run_bounded_robot.py','--problem',[string]$cfg.problem,'--method',$method,'--mode','practice','--base-url','http://127.0.0.1:2026','--robot-id',[string]$cfg.robot_id,'--confirm-practice')
  Log 'ROBOT_START' @{cwd='C:\BRobot';args=$arguments;case_code=$case}
  $env:PYTHONUTF8='1'
  $beforeRuns=@(Get-ChildItem 'C:\BRobot\robot_runs' -Directory | ForEach-Object Name)
  CheckSwitch
  $script:robot=Start-Process 'C:\Python314-arm64\python.exe' -ArgumentList $arguments -WorkingDirectory 'C:\BRobot' -RedirectStandardOutput "$JobDir\robot.stdout.log" -RedirectStandardError "$JobDir\robot.stderr.log" -WindowStyle Hidden -PassThru
  ...
  $newRuns=@(Get-ChildItem 'C:\BRobot\robot_runs' -Directory|Where-Object {$_.Name -notin $beforeRuns -and $_.Name -like '*-bounded-practice'})
