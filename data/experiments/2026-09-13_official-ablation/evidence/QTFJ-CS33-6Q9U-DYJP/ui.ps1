...
try {
 ...
 if($cfg.action -eq 'dry-run'){$script:outcome.ok=$true;$script:outcome.status='dry-run-ready'}
  ...
  $arguments=@('-B','official_ablation_robot.py','--variant','nearest_sensing__g50','--problem',[string]$cfg.problem,'--method',$method,'--mode','practice','--base-url','http://127.0.0.1:2026','--robot-id',[string]$cfg.robot_id,'--confirm-practice')
