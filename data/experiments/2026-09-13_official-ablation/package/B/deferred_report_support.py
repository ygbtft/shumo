...
def main():
    ...
    for problem,method in ((3,"range_area7"),(4,"range_width015")):
        data = []
        for name in ("coupled-confirmation","history-confirmation","deferred-confirmation"):
        ...
        assert len(group) == 450
        results.append(dict(problem=problem,method=method,cases=len(group),ordinary_seeds=list(range(117,147)),
    ...
    (OUT / "cross_batch_reference.json").write_text(json.dumps(dict(
        role="Descriptive aggregate of unchanged methods across three existing confirmation batches; not a new holdout or evidence for new deferred/prediction variants",
    ...
    stats = []
    for name in ("deferred-scans","batched-history","cheap-prediction","deferred-confirmation"):
        folder = OUT.parent / f"2026-09-11_{name}"
        ...
        stats.append(dict(run=name,scored=len(rows),all_cleared=sum(r["all_cleared"] for r in rows),
                          ...
                          deferred_skips=sum(r.get("certified_scan_skips",0) for r in rows),
                          pair_skips=sum(r.get("predicted_pair_skips",0) for r in rows)))
    (OUT / "stage_accounting.json").write_text(json.dumps(dict(runs=stats,new_scored_executions=8178,
        total_scored_executions=51894,previous_goal_turn="progress",current_goal_turn="progress",
        goal_status="active",ordinary_current_batch_thresholds_met=False,
    ...
    effects=[]
    for name,prefix in (("batched-history","batch_"),("cheap-prediction","cheap_")):
        folder=OUT.parent / f"2026-09-11_{name}"
        rows=[json.loads(line) for line in (folder / "trials.jsonl").read_text().splitlines()]
        lookup={(r["method"],r["case_id"]):r for r in rows}
        methods=sorted({r["method"] for r in rows if r["method"].startswith(prefix)})
        for method in methods:
            for split in ("ordinary","stress"):
                group=[r for r in rows if r["method"]==method and r["split"]==split]
                baseline=[lookup[method[len(prefix):],r["case_id"]] for r in group]
                assert all(a["total_virtual_s"]==b["total_virtual_s"] and a["commands"]==b["commands"] for a,b in zip(group,baseline))
                effects.append(dict(run=name,method=method,split=split,runs=len(group),identical_virtual_and_commands=True,
                                    cpu_reduction=1-sum(r["cpu_s"] for r in group)/sum(r["cpu_s"] for r in baseline),
                                    wall_reduction=1-sum(r["wall_s"] for r in group)/sum(r["wall_s"] for r in baseline)))
    (OUT / "training_cpu_effects.json").write_text(json.dumps(dict(comparisons=effects,scored_execution_count=0,official_calls=0),indent=2))
