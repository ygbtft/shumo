...
import numpy as np
from history_confirmation import OUT, SPECS
...
def main():
    ...
    assert completion["executions"] == completion["all_cleared"] == 2925
    rows = [json.loads(line) for line in (OUT / "trials.jsonl").read_text().splitlines()]
    summary = list(csv.DictReader((OUT / "summary.csv").open()))
    comparisons={3:tuple((m,b) for m in ("range_area7","faithful_area7") for b in ("area_return1_7","peer_layout9_proxy")),
                 4:tuple((m,b) for m in ("range_width015","faithful_width015","fast_arc05_width015","faithful_arc05_width015","faithful_locked_width015","history_first8_width015","history_recent8_width015")
                         for b in ("packet_center22","width40_f015_22","range_width015") if m!=b)
                    + (("faithful_arc05_width015","arc05_noskip_width015"),("faithful_locked_width015","locked_noskip_width015"))}
    ...
    for folder_name, title in (("historical-negatives","历史成对负反馈：已见训练比较"),("faithful-skips","保持原计划的静止RF删除：已见训练比较")):
