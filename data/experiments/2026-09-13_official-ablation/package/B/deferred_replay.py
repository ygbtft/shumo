"""Independent public-feedback and geometry checks for deferred scan deletion."""
import json
import math
import numpy as np
...
def expanded_plan(trace, spec, problem, paths, build):
    ...
    known_negatives = set()
    actual = []
    range_count = pair_count = nonstationary = 0
    ...
    def measure(q,ch):
        nonlocal index,range_count,pair_count,nonstationary
        ...
        if result == "certified_no_reception":
            ...
            assert was_surveying and poly is not None and ch not in policy.cleared
    ...
    assert stats["minimum_actual_unknown_per_station"] >= 4
    ...
    return dict(logical_commands=len(trace),actual_commands=client.sequence,skipped=len(skipped),
                range_skips=range_count,pair_skips=pair_count,nonstationary_skips=nonstationary,
