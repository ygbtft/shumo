"""21-station and lean/count policies over an owned ephemeral loopback HTTP service."""
import gzip
...
def main():
    ...
    paths = all_paths()
    checks = []
    for problem, method in ((3,"lean_deferred_area7"),(4,"range_grid21_29"),(4,"predict_grid21_29"),(4,"count_locked_grid21_29")):
        scenario = generate(42, ScenarioConfig(directional_fraction=.5 if problem == 4 else 0.))
        error = ErrorConfig()
        case_label = "default_seed42"
        if method == "count_locked_grid21_29":
            rows = [json.loads(line) for line in (OUT / "trials.jsonl").read_text().splitlines()]
            eligible = [r for r in rows if r["problem"] == 4 and r["method"] == method]
            chosen = max(eligible, key=lambda r: r.get("public_empty_scan_skips", 0))
            fixture = json.loads((OUT / "scenarios_scoring_only" / f"q4__{chosen['case_id']}.json").read_text())
            raw_scenario = fixture["scenario"]
            scenario = Scenario(raw_scenario["seed"], tuple(Source(**source) for source in raw_scenario["sources"]), {})
            error = ErrorConfig(**fixture["error"])
            case_label = chosen["case_id"]
        records = []
        for mode in ("inprocess", "owned_loopback_http"):
            ...
            try:
                ...
                records.append([(e["path"], e["request"], e["response"]["virtual_time_s"], e["response"].get("measure_result"),
                                 e["response"].get("svd_deg"), e["response"].get("clear_result")) for e in sim.trace])
                checks.append(dict(problem=problem, method=method, mode=mode, case_id=case_label, all_cleared=True,
                                   public_empty_skips=stats.get("public_empty_scan_skips",0),
