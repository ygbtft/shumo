...
def run_case(job):
    ...
    try:
        ...
        responses = [r for r in journal.rows if r.get('event') != 'transport_failure']
        assert len(responses) == len(sim.trace)
        assert all(r.get('http_status') == 200 and r.get('response', {}).get('accepted') is True for r in responses)
        assert all(r['request'] == t['request'] and r['response'] == t['response']
                   for r, t in zip(responses, sim.trace))
        by_id = {r['request']['request_id']: r for r in responses}
        assert all(r['request'] == by_id[r['request']['request_id']]['request']
                   for r in journal.rows if r.get('event') == 'transport_failure')
    except Exception:
